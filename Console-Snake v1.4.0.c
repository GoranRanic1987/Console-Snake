...
int   User_Def_Level = 1;
short User_Def_Delay = 12;
short User_Def_X_0 = 12, User_Def_X_1 = 12;
short User_Def_Y_0 = 12, User_Def_Y_1 = 13;
...
void Initialization (HDC DeviceContext, char *Argv)
{
 ...
 fscanf (Fp, "[%d:(%hd,%hd)%hd(%hd,%hd):%hd]",&User_Def_Level, &User_Def_X_0, &User_Def_Y_0, &User_Def_Direction, &User_Def_X_1, &User_Def_Y_1, &User_Def_Delay);
...
void Redraw_Screen (HDC DeviceContext)
{
 ...
 int Cnt;
 for (Cnt = 0; Cnt < Block_Length; Cnt++)
  Draw_Square (Block[Cnt].X, Block[Cnt].Y, DN, RGB (255,255,0), -1, -1, DN, RGB (255,255,255), 0, DeviceContext);
 Draw_Square (Cookie.X, Cookie.Y, DN, RGB (0,255,0), -1, -1, DN, RGB (255,255,255), 0, DeviceContext);
 Flag_Pause = False;
}
