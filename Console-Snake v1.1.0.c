...
void Initialization  (HDC DeviceContext);
...
int main (int argc, char *argv[]) 
{
 ...
 for (;;)
 {
  ...
  else if (Is_Snake (Draw.X, Draw.Y) == True)
  {
...
void Speed_Func (int Direction)
{
 if (Direction == UP && User_Def_Delay > 0)
  User_Def_Delay--;
...
void *Get_Key_Stroke (void)
{
 ...
 for(;;)
 {
  ...
  if (Key_Stroke_1 == 0xE0)
  {
   ...
   if      (Key_Stroke_2 == 0x48 && Move.Direction != DN)
    Predict_Func (UP);
   else if (Key_Stroke_2 == 0x50 && Move.Direction != UP)
    Predict_Func (DN);
   else if (Key_Stroke_2 == 0x4B && Move.Direction != RG)
    Predict_Func (LF);
   else if (Key_Stroke_2 == 0x4D && Move.Direction != LF)
    Predict_Func (RG);
   else
   {
    if (Key_Stroke_2 == 0x49)
     Speed_Func (UP);
...
void Clear_Screen (HDC DeviceContext)
{
 ...
 for (Cnt_X = 0; Cnt_X < 25; Cnt_X++)
  for (Cnt_Y = 0; Cnt_Y < 25; Cnt_Y++)
   Draw_Square (Cnt_X, Cnt_Y, RG, RGB( 0 , 0 , 0 ), -1, -1, DN, RGB(255,255,255), 0, DeviceContext);
}
